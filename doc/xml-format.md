...
Параметры модели описываются в следующих тегах:
  - `<precision>...</precision>` - точность времени в запускаемой модели 
    - `NS`, `MS`, `S`, `H`
  - `<duration>...</duration>` - длительность симлуияции
  - `<duration>...</duration>`
  - `<populate-routing-tables>...</populate-routing-tables>` - опция распространения таблиц маршрутов
